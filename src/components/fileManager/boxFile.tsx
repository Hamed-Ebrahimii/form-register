...
const BoxFile = ({type , file  , handleRemove } : IBoxFile) =>{
    return(
        ...
        <button type="button" className="absolute -top-1 -right-1 size-5 flex items-center justify-center text-white rounded-full bg-red-400" onClick={()=> handleRemove(file)}>
        <MdDelete />
        </button>
        </div>
    )
