...
export const parsNumberToString = (value: number) => {
  ...
  const fractional =
    (lengthNumber === 1 && Number(spliteNumber[1])) || +spliteNumber[2];
  const oneNumber = villagers
    ? Number(spliteNumber[1])
    : Number(spliteNumber[0]);
  ...
  if (villagers) {
    ...
    if (villagers >= 2) {
      if (oneNumber) {
        IntigertString =
          villagersStrings[villagers - 1] + " و " + oneStrings[oneNumber - 1];
      } else {
  ...
  if (fractional) {
    const oneNumberFractional =
      fractional % 10 === 0 ? 0 : Number(String(fractional / 10).split(".")[1]);
    ...
    if (villagersFractional) {
      ...
      if (
        villagersFractional > 0 &&
        villagersFractional < 2 &&
        oneNumberFractional > 0
      ) {
        IntigertString +=
          " و " + tenToTwenty[oneNumberFractional - 1] + " صدم ";
      }
      if (villagersFractional >= 2) {
        if (oneNumberFractional) {
          IntigertString +=
            " و " +
            villagersStrings[villagersFractional - 1] +
            " و " +
            oneStrings[oneNumberFractional - 1] +
            " صدم";
        } else {
